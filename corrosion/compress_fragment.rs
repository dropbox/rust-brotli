...
unsafe extern fn BrotliCompressFragmentFastImpl(
    ...
    let mut next_emit : usize = 0i32 as (usize);
    let base_ip : usize = 0i32 as (usize);
    ...
    let mut ip_index : usize = 0i32 as (usize);
    ...
    literal_ratio = BuildAndStoreLiteralPrefixCode(
                        m,
                        input_ptr.offset(input_index as (isize)),
                        block_size,
                            ...
                            'break15: loop {
                                {
                                    ...
                                    next_hash = Hash(input_ptr.offset(next_ip as (isize)),shift);
                                    candidate = ip_index.wrapping_sub(last_distance as (usize));
                                    if IsMatch(
                                           input_ptr.offset(ip_index as (isize)),
                                           input_ptr.offset(candidate as (isize))
                                       ) != 0 {
                                    ...
                                    candidate = base_ip.wrapping_add(
                                                    *table.offset(hash as (isize)) as (usize)
                                                );
                                    0i32;
                                    0i32;
                                    *table.offset(hash as (isize)) = ip_index.wrapping_sub(
                    ...
                    }
                    {
                        ...
                        let mut matched
                            ...
                            = (5i32 as (usize)).wrapping_add(
                                  FindMatchLengthWithLimit(
                                      input_ptr.offset(candidate as (isize)).offset(
                                          5i32 as (isize)
                                      ),
                                      input_ptr.offset(ip_index as (isize)).offset(5i32 as (isize)),
                                      ip_end.wrapping_sub(ip_index).wrapping_sub(5i32 as (usize))
                                  )
                              );
                        ...
                        if insert < 6210i32 as (usize) {
                            EmitInsertLen(
                                ...
                                storage
                                      ...
                                      input_ptr.offset(metablock_start as (isize)),
                                      input_ptr.offset(next_emit as (isize)),
                                      insert,
                                      literal_ratio
                                  ) != 0 {
                        ...
                        }
                        {
                            let mut input_bytes
                                : usize
                                = BROTLI_UNALIGNED_LOAD64(
                                      input_ptr.offset(ip_index as (isize)).offset(
                                          -(3i32 as (isize))
                                      ) as (*const ::std::os::raw::c_void)
                                  );
                            let mut prev_hash
                            ...
                            *table.offset(prev_hash as (isize)) = ip_index.wrapping_sub(
                            ...
                            candidate = base_ip.wrapping_add(
                                            *table.offset(cur_hash as (isize)) as (usize)
                                        );
                            *table.offset(cur_hash as (isize)) = ip_index.wrapping_sub(
                                                                     base_ip
                                                                 ) as (i32);
                        }
                    }

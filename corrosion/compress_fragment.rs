    ...
    if input_size < (1i32 << 15i32) as (usize) {
        ...
        'loop10: loop {
            if i < input_size {
                {
                    let _rhs = 1;
                    let _lhs = &mut histogram[*input.offset(i as (isize)) as (usize)];
...
unsafe extern fn ShouldMergeBlock(
    ...
    'loop1: loop {
        if i < len {
            {
                let _rhs = 1;
                let _lhs = &mut histo[*data.offset(i as (isize)) as (usize)];
    ...
    'loop3: loop {
        if i < 256i32 as (usize) {
            r = r - histo[i] as (f64) * (*depths.offset(
                                              i as (isize)
...
unsafe extern fn BrotliCompressFragmentFastImpl(
    ...
    if !(0i32 == 0) {
        ...
        i = 0i32 as (usize);
        'loop2: loop {
            if i.wrapping_add(7i32 as (usize)) < *cmd_code_numbits {
                BrotliWriteBits(
                    8i32 as (usize),
        ...
        'loop4: loop {
            if 1i32 != 0 {
                if code_block_selection as (i32) == CodeBlockState::EMIT_COMMANDS as (i32) {
                    memcpy(
                        cmd_histo.as_mut_ptr() as (*mut ::std::os::raw::c_void),
                        kCmdHistoSeed.as_mut_ptr() as (*const ::std::os::raw::c_void),
                        ...
                        'loop27: loop {
                            ...
                            if next_emit < ip {
                                ...
                                __assert_fail(
                                    ...
                                    file!().as_ptr(),
                                    line!(),
                                    (*b"BrotliCompressFragmentFastImpl\0").as_ptr()
                                );
                            }
                            'loop28: loop {
                                ...
                                if hash == Hash(next_ip,shift) {
                                    0i32;
                                } else {
                                    __assert_fail(
                                        (*b"hash == Hash(next_ip, shift)\0").as_ptr(),
                                        file!().as_ptr(),
                                        line!(),
                                        (*b"BrotliCompressFragmentFastImpl\0").as_ptr()
                                    );
                            ...
                            if code_block_selection as (i32) != CodeBlockState::EMIT_COMMANDS as (i32) {
                                ...
                                if ip >= ip_limit {
                                    ...
                                    let mut cur_hash
                                        : u32
                                        = HashBytesAtOffset(input_bytes,3i32,shift);
                                    *table.offset(
                                         prev_hash as (isize)
                                     ) = ((ip as (isize)).wrapping_sub(
                                              base_ip as (isize)
                    ...
                    if input_size > 0i32 as (usize) && (total_block_size.wrapping_add(
                        ...
                        if total_block_size > (1i32 << 16i32) as (usize) {
                            0i32;
                        } else {
                            __assert_fail(
                                (*b"total_block_size > (1 << 16)\0").as_ptr(),
                                file!().as_ptr(),
                                line!(),
                                (*b"BrotliCompressFragmentFastImpl\0").as_ptr()
                            );
                    ...
                    if 0i32 != 0 && (*b"Unrecognized code_block_selection\0" != 0) {
                        0i32;
                    } else {
                        __assert_fail(
                            (*b"0 && \"Unrecognized code_block_selection\"\0").as_ptr(),
                            file!().as_ptr(),
                            line!(),
                            (*b"BrotliCompressFragmentFastImpl\0").as_ptr()
                        );
    ...
    if input_size == 0i32 as (usize) {
        if is_last != 0 {
            ...
            __assert_fail(
                ...
                file!().as_ptr(),
                line!(),
                (*b"BrotliCompressFragmentFast\0").as_ptr()
            );
        }
        BrotliWriteBits(
            1i32 as (usize),

...
pub fn BrotliZopfliComputeShortestPath(
    ...
    while i.wrapping_add(HashTypeLengthH10()).wrapping_sub(
          ...
          ) < num_bytes {
        {
            ...
            if num_matches == 1 && (BackwardMatchLength(
                                                      &mut matches[(
                                                                0usize
                                                            ) ] 
                                                  ) > max_zopfli_len) {
        ...
        'continue56: loop {
            {
                ...
                if cost[(i as usize) ]< 1 as f32 {
                    cost[(i as usize) ]= 1 as f32;
                }
            }
            break;
        }
    ...
    while i.wrapping_add(3) < num_bytes {
        {
            ...
            if num_matches[(
                ...
                skip = brotli_max_size_t(
                           BackwardMatchLength(
                               &matches[(
                                     cur_match_pos.wrapping_sub(1) as usize
                                 ) ]
                           ),
                           skip
                       );
          ...
          ) < num_bytes {
        {
            ...
            if num_found_matches > 0usize {
                let match_len
                    ...
                    = BackwardMatchLength(
                          &mut matches[(
                                    cur_match_end.wrapping_sub(1) as usize
                                ) ] 
                      );

...
fn BuildAndStoreLiteralPrefixCode(mut m: &mut [MemoryManager],
                                  mut input: &[u8],
                                  input_size: usize,
                                  mut depths: &mut [u8],
                                  mut bits: &mut [u16],
...
fn Hash(mut p: &[u8], mut shift: usize) -> u32 {
  let h: usize = (BROTLI_UNALIGNED_LOAD64(p) << 24i32).wrapping_mul(kHashMul32 as (usize));
  (h >> shift) as (u32)
}
...
fn EmitInsertLen(mut insertlen: usize,
                 mut depth: &[u8],
                 mut bits: &[u16],
                 mut histo: &mut [u32],
                 mut storage_ix: &mut [usize],
...
fn ShouldUseUncompressedMode(mut metablock_start: &[u8],
                             mut next_emit: &[u8],
                             insertlen: usize,
                             literal_ratio: usize)
                             -> i32 {
  let compressed: usize = ((next_emit as (isize)).wrapping_sub(metablock_start as (isize)) /
                           ::std::mem::size_of::<*const u8>() as (isize)) as
                          (usize);
...
fn EmitUncompressedMetaBlock(mut begin: &[u8],
                             mut len: usize,
  ...
  *storage_ix = (*storage_ix).wrapping_add(7u32 as (usize)) & !7u32 as (usize);
  memcpy(&mut storage[((*storage_ix >> 3i32) as (usize))], begin, len);
  *storage_ix = (*storage_ix).wrapping_add(len << 3i32);
  storage[((*storage_ix >> 3i32) as (usize))] = 0i32 as (u8);
}

fn EmitLongInsertLen(mut insertlen: usize,
                     mut depth: &[u8],
                     mut bits: &[u16],
                     mut histo: &mut [u32],
                     mut storage_ix: &mut [usize],
...
fn EmitDistance(mut distance: usize,
                mut depth: &[u8],
                mut bits: &[u16],
                mut histo: &mut [u32],
                mut storage_ix: &mut [usize],
  ...
  let offset: usize = (2usize).wrapping_add(prefix) << nbits;
  let distcode: usize = ((2u32).wrapping_mul(nbits.wrapping_sub(1u32)) as (usize))
    .wrapping_add(prefix)
    .wrapping_add(80usize);
...
fn EmitCopyLenLastDistance(mut copylen: usize,
                           mut depth: &[u8],
                           mut bits: &[u16],
                           mut histo: &mut [u32],
                           mut storage_ix: &mut [usize],
  ...
  if copylen < 12usize {
    BrotliWriteBits(depth[(copylen.wrapping_sub(4usize) as (usize))] as (usize),
                    bits[(copylen.wrapping_sub(4usize) as (usize))] as (usize),
                    storage_ix,
                    storage);
    {
      let _rhs = 1;
    ...
    BrotliWriteBits(depth[(code as (usize))] as (usize),
                    ...
                    storage_ix,
                    storage);
    BrotliWriteBits(5usize, tail & 31usize, storage_ix, storage);
...
fn HashBytesAtOffset(mut v: usize, mut offset: i32, mut shift: usize) -> u32 {
  ...
  0i32;
  {
    let h: usize = (v >> 8i32 * offset << 24i32).wrapping_mul(kHashMul32 as (usize));
    (h >> shift) as (u32)
  }
}

fn EmitCopyLen(mut copylen: usize,
               mut depth: &[u8],
               mut bits: &[u16],
               mut histo: &mut [u32],
               mut storage_ix: &mut [usize],
  ...
  if copylen < 10usize {
    BrotliWriteBits(depth[(copylen.wrapping_add(14usize) as (usize))] as (usize),
                    bits[(copylen.wrapping_add(14usize) as (usize))] as (usize),
                    storage_ix,
                    storage);
    {
      let _rhs = 1;
...
fn ShouldMergeBlock(mut data: &[u8], mut len: usize, mut depths: &[u8]) -> i32 {
  let mut histo: [usize; 256] =
  ...
  }
  {
    ...
    let mut r: f64 = (FastLog2(total) + 0.5f64) * total as (f64) + 200i32 as (f64);
    i = 0usize;
    while i < 256usize {
      {
        r = r - histo[i] as (f64) * (depths[(i as (usize))] as (f64) + FastLog2(histo[i]));
      }
      i = i.wrapping_add(1 as (usize));
    }
...
fn UpdateBits(mut n_bits: usize, mut bits: u32, mut pos: usize, mut array: &mut [u8]) {
  while n_bits > 0usize {
    ...
    let mut n_changed_bits: usize = brotli_min_size_t(n_bits,
                                                      (8usize).wrapping_sub(n_unchanged_bits));
    let mut total_bits: usize = n_unchanged_bits.wrapping_add(n_changed_bits);
    let mut mask: u32 = !(1u32 << total_bits).wrapping_sub(1u32) |
                        (1u32 << n_unchanged_bits).wrapping_sub(1u32);
    let mut unchanged_bits: u32 = array[(byte_pos as (usize))] as (u32) & mask;
    let mut changed_bits: u32 = bits & (1u32 << n_changed_bits).wrapping_sub(1u32);
...
fn BuildAndStoreCommandPrefixCode(mut histogram: &[u32],
  ...
  BrotliCreateHuffmanTree(&histogram[(64usize)],
                          ...
                          tree.as_mut_ptr(),
                          &mut depth[(64usize)]);
  memcpy(cmd_depth.as_mut_ptr(), depth, 24usize);
  ...
  memcpy(bits, cmd_bits.as_mut_ptr(), 48usize);
  memcpy(bits[(24usize)..],
  ...
  BrotliConvertBitDepthsToSymbols(&mut depth[(64usize)], 64usize, &mut bits[(64usize)]);
  {
    ...
    memcpy(cmd_depth.as_mut_ptr(), depth, 8usize);
...
fn BrotliCompressFragmentFastImpl(mut m: &mut [MemoryManager],
                                  mut input_ptr: &[u8],
  ...
  BrotliWriteBits(*cmd_code_numbits & 7usize,
                  cmd_code[((*cmd_code_numbits >> 3i32) as (usize))] as (usize),
                  storage_ix,
                  storage);
        ...
        loop {
          ...
          }
          {
            ...
            let mut matched: usize =
              (5usize).wrapping_add(FindMatchLengthWithLimit(input_ptr[(candidate as (usize))..]
            ...
            if insert < 6210usize {
              EmitInsertLen(insert,
                            ...
                            storage_ix,
                            storage);
            } else if ShouldUseUncompressedMode(input_ptr[(metablock_start as (usize))..],
            ...
            {
              let mut input_bytes: usize =
                BROTLI_UNALIGNED_LOAD64(input_ptr[(ip_index as (usize))..].offset(-(3i32 as
                                                                                    (isize))));

extern "C" {
  ...
  fn BrotliEstimateBitCostsForLiterals(pos: usize,
                                       len: usize,
                                       mask: usize,
                                       data: &[u8],
                                       cost: &mut [f32]);
...
static mut kDistanceCacheIndex: [u32; 16] = [0u32, 1u32, 2u32, 3u32, 0u32, 0u32, 0u32, 0u32, 0u32,
                                             0u32, 1u32, 1u32, 1u32, 1u32, 1u32, 1u32];

static mut kDistanceCacheOffset: [i32; 16] = [0i32, 0i32, 0i32, 0i32, -1i32, 1i32, -2i32, 2i32,
                                              -3i32, 3i32, -1i32, 1i32, -2i32, 2i32, -3i32, 3i32];
...
pub fn BrotliInitZopfliNodes(mut array: &mut [ZopfliNode], mut length: usize) {
  let mut stub: ZopfliNode;
  let mut i: usize;
  stub.length = 1u32;
  stub.distance = 0u32;
  stub.insert_length = 0u32;
...
fn ZopfliNodeCopyLength(mut xself: &ZopfliNode) -> u32 {
  (*xself).length & 0xffffffu32
}

fn ZopfliNodeCopyDistance(mut xself: &ZopfliNode) -> u32 {
  (*xself).distance & 0x1ffffffu32
}

fn ZopfliNodeLengthCode(mut xself: &ZopfliNode) -> u32 {
  let modifier: u32 = (*xself).length >> 24i32;
  ZopfliNodeCopyLength(xself).wrapping_add(9u32).wrapping_sub(modifier)
}
...
fn ZopfliNodeDistanceCode(mut xself: &ZopfliNode) -> u32 {
  let short_code: u32 = (*xself).distance >> 25i32;
  if short_code == 0u32 {
    ZopfliNodeCopyDistance(xself).wrapping_add(16u32).wrapping_sub(1u32)
  } else {
...
pub fn BrotliZopfliCreateCommands(num_bytes: usize,
                                  block_start: usize,
  ...
  while offset != !(0u32) {
    {
      let mut next: *const ZopfliNode = &nodes[(pos.wrapping_add(offset as (usize)) as (usize))];
      let mut copy_length: usize = ZopfliNodeCopyLength(next) as (usize);
      ...
      }
      {
        ...
        let mut max_distance: usize = brotli_min_size_t(block_start.wrapping_add(pos),
                                                        max_backward_limit);
        let mut is_dictionary: i32 = if !!(distance > max_distance) {
          1i32
        } else {
          0i32
        };
        let mut dist_code: usize = ZopfliNodeDistanceCode(next) as (usize);
        InitCommand(&mut commands[(i as (usize))],
                    insert_length,
                    copy_length,
                    len_code,
                    dist_code);
...
fn MaxZopfliLen(mut params: &[BrotliEncoderParams]) -> usize {
...
fn InitZopfliCostModel(mut m: &mut [MemoryManager],
                       mut xself: &mut ZopfliCostModel,
                       mut num_bytes: usize) {
  (*xself).num_bytes_ = num_bytes;
  (*xself).literal_costs_ = if num_bytes.wrapping_add(2usize) != 0 {
    BrotliAllocate(m,
                   num_bytes.wrapping_add(2usize).wrapping_mul(::std::mem::size_of::<f32>()))
  } else {
    0i32
  };
  if !(0i32 == 0) {}
...
fn ZopfliCostModelSetFromLiteralCosts(mut xself: &mut ZopfliCostModel,
                                      mut position: usize,
                                      mut ringbuffer: &[u8],
                                      mut ringbuffer_mask: usize) {
  let mut literal_costs: *mut f32 = (*xself).literal_costs_;
  let mut cost_dist: *mut f32 = (*xself).cost_dist_.as_mut_ptr();
  let mut cost_cmd: *mut f32 = (*xself).cost_cmd_.as_mut_ptr();
  let mut num_bytes: usize = (*xself).num_bytes_;
  let mut i: usize;
  BrotliEstimateBitCostsForLiterals(position,
                                    num_bytes,
                                    ringbuffer_mask,
                                    ringbuffer,
  ...
  while i < num_bytes {
    {
      let _rhs = literal_costs[(i as (usize))];
      let _lhs = &mut literal_costs[(i.wrapping_add(1usize) as (usize))];
      *_lhs = *_lhs + _rhs;
    }
    i = i.wrapping_add(1 as (usize));
  }
  ...
  while i < 704usize {
    {
      cost_cmd[(i as (usize))] = FastLog2((11u32).wrapping_add(i as (u32)) as (usize)) as (f32);
    }
    i = i.wrapping_add(1 as (usize));
  }
  i = 0usize;
  while i < 520usize {
    {
      cost_dist[(i as (usize))] = FastLog2((20u32).wrapping_add(i as (u32)) as (usize)) as (f32);
    }
    i = i.wrapping_add(1 as (usize));
  }
...
fn GetHasherCommon(mut handle: &mut [u8]) -> *mut Struct18 {
  handle
}
...
fn InitDictionaryBackwardMatch(mut xself: &mut BackwardMatch,
                               mut dist: usize,
                               mut len: usize,
                               mut len_code: usize) {
  (*xself).distance = dist as (u32);
  (*xself).length_and_code = (len << 5i32 | if len == len_code { 0usize } else { len_code }) as
                             (u32);
}

fn FindAllMatchesH10(mut handle: &mut [u8],
  ...
  if best_len < max_length {
    matches = StoreAndFindMatchesH10(SelfH10(handle),
                                     data,
                                     cur_ix,
                                     ring_buffer_mask,
                                     max_length,
      ...
      while l <= maxlen {
        {
          ...
          if dict_id < kInvalidMatch {
            InitDictionaryBackwardMatch({
                                        ...
                                        max_backward.wrapping_add((dict_id >> 5i32) as (usize))
                                          .wrapping_add(1usize),
                                        l,
  ...
  ((matches as (isize)).wrapping_sub(orig_matches as (isize)) /
   ::std::mem::size_of::<*mut BackwardMatch>() as (isize)) as (usize)
}

fn BackwardMatchLength(mut xself: &BackwardMatch) -> usize {
  ((*xself).length_and_code >> 5i32) as (usize)
}

fn MaxZopfliCandidates(mut params: &[BrotliEncoderParams]) -> usize {
  (if (*params).quality <= 10i32 {
     1i32
   } else {
     5i32
...
fn ComputeDistanceShortcut(block_start: usize,
                           pos: usize,
                           max_backward: usize,
                           mut nodes: &[ZopfliNode])
                           -> u32 {
  let clen: usize = ZopfliNodeCopyLength(&nodes[(pos as (usize))]) as (usize);
  let ilen: usize = (nodes[(pos as (usize))]).insert_length as (usize);
  let dist: usize = ZopfliNodeCopyDistance(&nodes[(pos as (usize))]) as (usize);
  if pos == 0usize {
    0u32
  } else if dist.wrapping_add(clen) <= block_start.wrapping_add(pos) && (dist <= max_backward) &&
            (ZopfliNodeDistanceCode(&nodes[(pos as (usize))]) > 0u32) {
    pos as (u32)
  } else {
    (nodes[(pos.wrapping_sub(clen).wrapping_sub(ilen) as (usize))]).u.shortcut
  }
}

fn ZopfliCostModelGetLiteralCosts(mut xself: &ZopfliCostModel,
                                  mut from: usize,
                                  mut to: usize)
                                  -> f32 {
  *(*xself).literal_costs_[(to as (usize))..] - *(*xself).literal_costs_[(from as (usize))..]
}
fn ComputeDistanceCache(pos: usize,
                        mut starting_dist_cache: &[i32],
                        mut nodes: &[ZopfliNode],
                        mut dist_cache: &mut [i32]) {
  let mut idx: i32 = 0i32;
  let mut p: usize = (nodes[(pos as (usize))]).u.shortcut as (usize);
  while idx < 4i32 && (p > 0usize) {
    let ilen: usize = (nodes[(p as (usize))]).insert_length as (usize);
    ...
    p = (nodes[(p.wrapping_sub(clen).wrapping_sub(ilen) as (usize))]).u.shortcut as (usize);
  }
  while idx < 4i32 {
    {
      dist_cache[(idx as (usize))] = *{
                                        let _old = starting_dist_cache;
                                        starting_dist_cache = starting_dist_cache[(1 as (usize))..];
                                        _old
                                      };
    }
    idx = idx + 1;
...
fn StartPosQueueSize(mut xself: &StartPosQueue) -> usize {
  brotli_min_size_t((*xself).idx_, 8usize)
}

fn StartPosQueuePush(mut xself: &mut StartPosQueue, mut posdata: &[PosData]) {
  let mut offset: usize = !{
                             let _old = (*xself).idx_;
                             (*xself).idx_ = (*xself).idx_.wrapping_add(1 as (usize));
                             _old
                           } & 7usize;
  let mut len: usize = StartPosQueueSize(xself);
  let mut i: usize;
  let mut q: *mut PosData = (*xself).q_.as_mut_ptr();
...
fn EvaluateNode(block_start: usize,
                pos: usize,
                max_backward_limit: usize,
                mut starting_dist_cache: &[i32],
  ...
  (nodes[(pos as (usize))]).u.shortcut =
    ComputeDistanceShortcut(block_start, pos, max_backward_limit, nodes);
  if node_cost <= ZopfliCostModelGetLiteralCosts(model, 0usize, pos) {
    let mut posdata: PosData;
    posdata.pos = pos;
    posdata.cost = node_cost;
    posdata.costdiff = node_cost - ZopfliCostModelGetLiteralCosts(model, 0usize, pos);
    ComputeDistanceCache(pos,
                         starting_dist_cache,
                         nodes,
...
fn StartPosQueueAt(mut xself: &StartPosQueue, mut k: usize) -> *const PosData {
  &mut (*xself).q_[k.wrapping_sub((*xself).idx_) & 7usize]
}

fn ZopfliCostModelGetMinCostCmd(mut xself: &ZopfliCostModel) -> f32 {
  (*xself).min_cost_cmd_
}

fn ComputeMinimumCopyLength(start_cost: f32,
                            mut nodes: &[ZopfliNode],
                            num_bytes: usize,
                            pos: usize)
                            -> usize {
  ...
  while pos.wrapping_add(len) <= num_bytes &&
        ((nodes[(pos.wrapping_add(len) as (usize))]).u.cost <= min_cost) {
...
fn ZopfliCostModelGetDistanceCost(mut xself: &ZopfliCostModel, mut distcode: usize) -> f32 {
  (*xself).cost_dist_[distcode]
}
...
fn ZopfliCostModelGetCommandCost(mut xself: &ZopfliCostModel, mut cmdcode: u16) -> f32 {
  (*xself).cost_cmd_[cmdcode as (usize)]
}

fn UpdateZopfliNode(mut nodes: &mut [ZopfliNode],
                    mut pos: usize,
                    mut start_pos: usize,
                    mut len: usize,
                    mut len_code: usize,
                    ...
                    mut cost: f32) {
  let mut next: *mut ZopfliNode = &mut nodes[(pos.wrapping_add(len) as (usize))];
  (*next).length = (len | len.wrapping_add(9u32 as (usize)).wrapping_sub(len_code) << 24i32) as
                   (u32);
...
fn BackwardMatchLengthCode(mut xself: &BackwardMatch) -> usize {
  let mut code: usize = ((*xself).length_and_code & 31u32) as (usize);
  if code != 0 {
    code
  } else {
...
fn UpdateNodes(num_bytes: usize,
               block_start: usize,
               pos: usize,
               mut ringbuffer: &[u8],
  ...
  EvaluateNode(block_start,
               ...
               nodes);
  {
    let mut posdata: *const PosData = StartPosQueueAt(queue, 0usize);
    let mut min_cost: f32 = (*posdata).cost + ZopfliCostModelGetMinCostCmd(model) +
  ...
  while k < max_iters && (k < StartPosQueueSize(queue)) {
    'continue28: loop {
      {
        let mut posdata: *const PosData = StartPosQueueAt(queue, k);
        ...
        let base_cost: f32 = start_costdiff + GetInsertExtra(inscode) as (f32) +
                             ZopfliCostModelGetLiteralCosts(model, 0usize, pos);
        let mut best_len: usize = min_len.wrapping_sub(1usize);
        let mut j: usize = 0usize;
        while j < 16usize && (best_len < max_len) {
          'continue30: loop {
            {
              let idx: usize = kDistanceCacheIndex[j] as (usize);
              let backward: usize = ((*posdata).distance_cache[idx] + kDistanceCacheOffset[j]) as
                                    (usize);
              ...
              if cur_ix_masked.wrapping_add(best_len) > ringbuffer_mask ||
                 prev_ix.wrapping_add(best_len) > ringbuffer_mask ||
                 ringbuffer[(cur_ix_masked.wrapping_add(best_len) as (usize))] as (i32) !=
              ...
              {
                let len: usize = FindMatchLengthWithLimit(&ringbuffer[(prev_ix as (usize))],
                ...
                while l <= len {
                  {
                    ...
                    let cost: f32 = (if cmdcode as (i32) < 128i32 {
                                       base_cost
                                     } else {
                                       dist_cost
                                     }) +
                                    GetCopyExtra(copycode) as (f32) +
                                    ZopfliCostModelGetCommandCost(model, cmdcode);
                    if cost < (nodes[(pos.wrapping_add(l) as (usize))]).u.cost {
                      UpdateZopfliNode(nodes,
                                       pos,
                                       start,
                                       l,
                                       l,
          ...
          while j < num_matches {
            {
              ...
              let mut is_dictionary_match: i32 = if !!(dist > max_distance) { 1i32 } else { 0i32 };
              let mut dist_code: usize = dist.wrapping_add(16usize).wrapping_sub(1usize);
              let mut dist_symbol: u16;
              let mut distextra: u32;
              let mut distnumextra: u32;
              let mut dist_cost: f32;
              let mut max_match_len: usize;
              PrefixEncodeCopyDistance(dist_code, 0usize, 0usize, &mut dist_symbol, &mut distextra);
              distnumextra = distextra >> 24i32;
              dist_cost = base_cost + distnumextra as (f32) +
                          ZopfliCostModelGetDistanceCost(model, dist_symbol as (usize));
              ...
              while len <= max_match_len {
                {
                  ...
                  let cost: f32 = dist_cost + GetCopyExtra(copycode) as (f32) +
                                  ZopfliCostModelGetCommandCost(model, cmdcode);
                  if cost < (nodes[(pos.wrapping_add(len) as (usize))]).u.cost {
...
fn StoreRangeH10(mut handle: &mut [u8],
                 mut data: &[u8],
                 mask: usize,
                 ix_start: usize,
                 ix_end: usize) {
...
fn CleanupZopfliCostModel(mut m: &mut [MemoryManager], mut xself: &mut ZopfliCostModel) {
  BrotliFree(m, (*xself).literal_costs_);
  (*xself).literal_costs_ = 0i32;
}

fn ZopfliNodeCommandLength(mut xself: &ZopfliNode) -> u32 {
  ZopfliNodeCopyLength(xself).wrapping_add((*xself).insert_length)
}

fn ComputeShortestPathFromNodes(mut num_bytes: usize, mut nodes: &mut [ZopfliNode]) -> usize {
  let mut index: usize = num_bytes;
  let mut num_commands: usize = 0usize;
  while (nodes[(index as (usize))]).insert_length == 0u32 &&
        ((nodes[(index as (usize))]).length == 1u32) {
    index = index.wrapping_sub(1 as (usize));
  }
  (nodes[(index as (usize))]).u.next = !(0u32);
  while index != 0usize {
    let mut len: usize = ZopfliNodeCommandLength(&mut nodes[(index as (usize))]) as (usize);
...
pub fn BrotliZopfliComputeShortestPath(mut m: &mut [MemoryManager],
                                       mut dictionary: &[BrotliDictionary],
                                       mut num_bytes: usize,
                                       mut position: usize,
                                       mut ringbuffer: &[u8],
  ...
  let store_end: usize = if num_bytes >= StoreLookaheadH10() {
    position.wrapping_add(num_bytes).wrapping_sub(StoreLookaheadH10()).wrapping_add(1usize)
  } else {
    position
  };
  let mut i: usize;
  ...
  InitZopfliCostModel(m, &mut model, num_bytes);
  if !(0i32 == 0) {
    return 0usize;
  }
  ...
  i = 0usize;
  while i.wrapping_add(HashTypeLengthH10()).wrapping_sub(1usize) < num_bytes {
    {
      let pos: usize = position.wrapping_add(i);
      let max_distance: usize = brotli_min_size_t(pos, max_backward_limit);
      let mut num_matches: usize = FindAllMatchesH10(hasher,
                                                     dictionary,
                                                     ringbuffer,
                                                     ringbuffer_mask,
                                                     pos,
      ...
      if num_matches > 0usize &&
         (BackwardMatchLength(&mut matches[num_matches.wrapping_sub(1usize)]) > max_zopfli_len) {
        matches[0usize] = matches[num_matches.wrapping_sub(1usize)];
        num_matches = 1usize;
      }
      skip = UpdateNodes(num_bytes,
                         position,
                         i,
                         ringbuffer,
                         ringbuffer_mask,
      ...
      if num_matches == 1usize && (BackwardMatchLength(&mut matches[0usize]) > max_zopfli_len) {
        skip = brotli_max_size_t(BackwardMatchLength(&mut matches[0usize]), skip);
      }
      if skip > 1usize {
        ...
        while skip != 0 {
          i = i.wrapping_add(1 as (usize));
          if i.wrapping_add(HashTypeLengthH10()).wrapping_sub(1usize) >= num_bytes {
            {
              break;
            }
          }
...
pub fn BrotliCreateZopfliBackwardReferences(mut m: &mut [MemoryManager],
                                            mut dictionary: &[BrotliDictionary],
                                            mut num_bytes: usize,
                                            mut position: usize,
                                            mut ringbuffer: &[u8],
  ...
  nodes = if num_bytes.wrapping_add(1usize) != 0 {
    BrotliAllocate(m,
                   num_bytes.wrapping_add(1usize).wrapping_mul(::std::mem::size_of::<ZopfliNode>()))
  } else {
    0i32
  };
  if !(0i32 == 0) {
  ...
  BrotliInitZopfliNodes(nodes, num_bytes.wrapping_add(1usize));
  *num_commands =
    (*num_commands).wrapping_add(BrotliZopfliComputeShortestPath(m,
                                                                 dictionary,
                                                                 num_bytes,
                                                                 position,
                                                                 ringbuffer,
  ...
  BrotliZopfliCreateCommands(num_bytes,
                             position,
                             max_backward_limit,
                             nodes,
                             dist_cache,
...
fn SetCost(mut histogram: &[u32], mut histogram_size: usize, mut cost: &mut [f32]) {
    ...
    'continue54: loop {
      {
        if histogram[(i as (usize))] == 0u32 {
          cost[(i as (usize))] = log2sum + 2i32 as (f32);
          {
            {
              break 'continue54;
            }
        ...
        cost[(i as (usize))] = log2sum - FastLog2(histogram[(i as (usize))] as (usize)) as (f32);
...
fn brotli_min_float(mut a: f32, mut b: f32) -> f32 {
  if a < b { a } else { b }
}

fn ZopfliCostModelSetFromCommands(mut xself: &mut ZopfliCostModel,
                                  mut position: usize,
                                  mut ringbuffer: &[u8],
                                  mut ringbuffer_mask: usize,
  ...
  let mut i: usize;
  let mut cost_cmd: *mut f32 = (*xself).cost_cmd_.as_mut_ptr();
  memset(histogram_literal.as_mut_ptr(),
         0i32,
         ::std::mem::size_of::<[u32; 256]>());
  memset(histogram_cmd.as_mut_ptr(),
         0i32,
         ::std::mem::size_of::<[u32; 704]>());
  memset(histogram_dist.as_mut_ptr(),
         0i32,
         ::std::mem::size_of::<[u32; 520]>());
  i = 0usize;
  while i < num_commands {
    {
      let mut inslength: usize = (commands[(i as (usize))]).insert_len_ as (usize);
      let mut copylength: usize = CommandCopyLen(&commands[(i as (usize))]) as (usize);
      let mut distcode: usize = (commands[(i as (usize))]).dist_prefix_ as (usize);
      let mut cmdcode: usize = (commands[(i as (usize))]).cmd_prefix_ as (usize);
      let mut j: usize;
      {
        let _rhs = 1;
      ...
      while j < inslength {
        {
          let _rhs = 1;
          let _lhs = &mut histogram_literal[ringbuffer[((pos.wrapping_add(j) & ringbuffer_mask) as
                           (usize))] as (usize)];
          *_lhs = (*_lhs).wrapping_add(_rhs as (u32));
        }
        j = j.wrapping_add(1 as (usize));
  ...
  SetCost(histogram_literal.as_mut_ptr(),
          256usize,
          cost_literal.as_mut_ptr());
  SetCost(histogram_cmd.as_mut_ptr(), 704usize, cost_cmd);
  SetCost(histogram_dist.as_mut_ptr(),
          520usize,
          (*xself).cost_dist_.as_mut_ptr());
  i = 0usize;
  while i < 704usize {
    {
    ...
    while i < num_bytes {
      {
        literal_costs[(i.wrapping_add(1usize) as (usize))] =
          ...
          cost_literal[ringbuffer[((position.wrapping_add(i) & ringbuffer_mask) as (usize))] as
          (usize)];
      }
      i = i.wrapping_add(1 as (usize));
    }
...
fn ZopfliIterate(mut num_bytes: usize,
                 mut position: usize,
                 mut ringbuffer: &[u8],
                 mut ringbuffer_mask: usize,
                 mut params: &[BrotliEncoderParams],
  ...
  while i.wrapping_add(3usize) < num_bytes {
    {
      let mut skip: usize = UpdateNodes(num_bytes,
                                        position,
                                        i,
                                        ringbuffer,
                                        ringbuffer_mask,
      ...
      if num_matches[(i as (usize))] == 1u32 &&
         (BackwardMatchLength(&matches[(cur_match_pos.wrapping_sub(1usize) as (usize))]) >
          max_zopfli_len) {
        skip = brotli_max_size_t(BackwardMatchLength(&matches[(cur_match_pos.wrapping_sub(1usize) as
                                                       (usize))]),
                                 skip);
      }
      if skip > 1usize {
...
pub fn BrotliCreateHqZopfliBackwardReferences(mut m: &mut [MemoryManager],
                                              mut dictionary: &[BrotliDictionary],
                                              mut num_bytes: usize,
                                              mut position: usize,
                                              mut ringbuffer: &[u8],
                                              ...
                                              mut num_literals: &mut [usize]) {
  let max_backward_limit: usize = (1usize << (*params).lgwin).wrapping_sub(16usize);
  let mut num_matches: *mut u32 = if num_bytes != 0 {
    BrotliAllocate(m, num_bytes.wrapping_mul(::std::mem::size_of::<u32>()))
  } else {
    0i32
  };
  ...
  let mut model: ZopfliCostModel;
  let mut nodes: *mut ZopfliNode;
  let mut matches: *mut BackwardMatch = if matches_size != 0 {
    BrotliAllocate(m,
  ...
  while i.wrapping_add(HashTypeLengthH10()).wrapping_sub(1usize) < num_bytes {
    {
      let pos: usize = position.wrapping_add(i);
      let mut max_distance: usize = brotli_min_size_t(pos, max_backward_limit);
      let mut max_length: usize = num_bytes.wrapping_sub(i);
      let mut num_found_matches: usize;
      let mut cur_match_end: usize;
      let mut j: usize;
      {
        if matches_size < cur_match_pos.wrapping_add(128usize) {
          let mut _new_size: usize = if matches_size == 0usize {
            cur_match_pos.wrapping_add(128usize)
          } else {
            matches_size
          };
          let mut new_array: *mut BackwardMatch;
          while _new_size < cur_match_pos.wrapping_add(128usize) {
      ...
      num_found_matches = FindAllMatchesH10(hasher,
                                            ...
                                            max_distance,
                                            params,
                                            &mut matches[(cur_match_pos as (usize))]);
      ...
      if num_found_matches > 0usize {
        let match_len: usize = BackwardMatchLength(&mut matches[(cur_match_end.wrapping_sub(1usize) as
                                                         (usize))]);
        if match_len > 325usize {
          ...
          matches[({
             ...
             _old
           } as (usize))] = matches[(cur_match_end.wrapping_sub(1usize) as (usize))];
          ...
          memset(&mut num_matches[(i.wrapping_add(1usize) as (usize))],
                 0i32,
  ...
  memcpy(orig_dist_cache.as_mut_ptr(),
         dist_cache,
  ...
  while i < 2usize {
    {
      ...
      if i == 0usize {
        ...
        ZopfliCostModelSetFromCommands(&mut model,
                                       position,
                                       ringbuffer,
                                       ringbuffer_mask,
                                       commands,
      ...
      *last_insert_len = orig_last_insert_len;
      memcpy(dist_cache,
             orig_dist_cache.as_mut_ptr(),
             (4usize).wrapping_mul(::std::mem::size_of::<i32>()));
      *num_commands = (*num_commands).wrapping_add(ZopfliIterate(num_bytes,
                                                                 position,
                                                                 ringbuffer,
                                                                 ringbuffer_mask,
                                                                 params,

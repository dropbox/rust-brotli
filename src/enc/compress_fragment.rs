    ...
    while i < 256usize {
      {
        if histogram[i] != 0 {
          literal_ratio = literal_ratio
            .wrapping_add(histogram[i].wrapping_mul(depths[(i as (usize))] as (u32)) as (usize));
        }
      }
      i = i.wrapping_add(1 as (usize));
    }
...
fn UpdateBits(mut n_bits: usize, mut bits: u32, mut pos: usize, array: &mut [u8]) {
  while n_bits > 0usize {
    ...
    let mask: u32 =
      !(1u32 << total_bits).wrapping_sub(1u32) | (1u32 << n_unchanged_bits).wrapping_sub(1u32);
  ...
  loop {
    ...
    if code_block_selection == CodeBlockState::EMIT_COMMANDS {
      ...
      if block_size >= kInputMarginBytes {
        ...
        loop {
          ...
          while IsMatch(&input_ptr[ip_index..], &input_ptr[candidate..]) != 0 {
            ...
            if ip_index.wrapping_sub(candidate) > (1usize << 18i32).wrapping_sub(16usize) {
              break;
            }
            ip_index = ip_index.wrapping_add(matched);
            ...
            {
              assert!(ip_index >= 3);
              let input_bytes: u64 = BROTLI_UNALIGNED_LOAD64(&input_ptr[ip_index as usize - 3..]);
              let mut prev_hash: u32 = HashBytesAtOffset(input_bytes, 0i32, shift);
              let cur_hash: u32 = HashBytesAtOffset(input_bytes, 3i32, shift);
              table[prev_hash as usize] =
                ip_index.wrapping_sub(base_ip).wrapping_sub(3usize) as i32;
      ...
      if input_size > 0
        ...
        && (ShouldMergeBlock(&input_ptr[input_index..], block_size, &mut lit_depth[..]) != 0)
      {

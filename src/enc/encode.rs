...
impl<Alloc: BrotliAlloc> BrotliEncoderStateStruct<Alloc> {
    ...
    pub fn set_custom_dictionary_with_optional_precomputed_hasher(
        ...
        if dict_size == 0usize
            || self.params.quality == 0i32
            || self.params.quality == 1i32
            || size <= 1
        {
...
impl<Alloc: BrotliAlloc> BrotliEncoderStateStruct<Alloc> {
    fn InjectBytePaddingBlock(&mut self) {
    ...
    fn InjectFlushOrPushOutput(
        ...
        {
            self.InjectBytePaddingBlock();
            return 1i32;
        }
        if self.available_out_ != 0usize && (*available_out != 0usize) {
            let copy_output_size: usize = brotli_min_size_t(self.available_out_, *available_out);
    ...
    fn UnprocessedInputSize(&self) -> u64 {
        self.input_pos_.wrapping_sub(self.last_processed_pos_)
    }

    fn UpdateSizeHint(&mut self, available_in: usize) {
        if self.params.size_hint == 0usize {
            let delta: u64 = self.UnprocessedInputSize();
...
impl<Alloc: BrotliAlloc> BrotliEncoderStateStruct<Alloc> {
    fn GetBrotliStorage(&mut self, size: usize) {
...
impl<Alloc: BrotliAlloc> BrotliEncoderStateStruct<Alloc> {
    ...
    {
        let mut delta: u64 = self.UnprocessedInputSize();
        ...
        let mut storage_ix: usize = usize::from(self.last_bytes_bits_);
        {
            ...
            self.GetBrotliStorage((2usize).wrapping_mul(meta_size).wrapping_add(503 + 24));
        }
        {
        ...
        if let IsFirst::BothCatableBytesWritten = self.is_first_mb {
            ...
            self.next_out_ = NextOut::DynamicStorage(0);
            *out_size = catable_header_size;
            delta = self.UnprocessedInputSize();
        }
...
impl<Alloc: BrotliAlloc> BrotliEncoderStateStruct<Alloc> {
    ...
    >(
        ...
        loop {
            if self.InjectFlushOrPushOutput(
                available_out,
                next_out_array,
                next_out_offset,
                total_out,
                ...
                == BrotliEncoderStreamState::BROTLI_STREAM_METADATA_HEAD as i32
            {
                ...
                if *available_out != 0 {
                    let copy: u32 =
                        brotli_min_size_t(self.remaining_metadata_bytes_ as usize, *available_out)
                            as u32;
                    ...
                    *available_out = available_out.wrapping_sub(copy as usize);
                    ...
                    let copy: u32 = brotli_min_uint32_t(self.remaining_metadata_bytes_, 16u32);
                    self.next_out_ = NextOut::TinyBuf(0);
...
impl<Alloc: BrotliAlloc> BrotliEncoderStateStruct<Alloc> {
        ...
        loop {
                    ...
                    || op as i32 != BrotliEncoderOperation::BROTLI_OPERATION_PROCESS as i32)
            {
                ...
                if max_out_size <= *available_out {
                    ...
                    inplace = 0i32;
                    self.GetBrotliStorage(max_out_size);
    ...
    >(
        ...
        loop {
                    ...
                    || op as i32 != BrotliEncoderOperation::BROTLI_OPERATION_PROCESS as i32)
            {
                ...
                self.UpdateSizeHint(*available_in);
    ...
    pub fn take_output(&mut self, size: &mut usize) -> &[u8] {
        ...
        if *size != 0 {
            consumed_size = brotli_min_size_t(*size, self.available_out_);
        }
        if consumed_size != 0 {

...
fn get_stride_cdf_low(
  ...
  let index: usize = 1
    + 2
      * (cm_prior as usize | ((stride_prior as usize & 0xf) << 8) | ((high_nibble as usize) << 12));
  data
    .split_at_mut(NUM_SPEEDS_TO_TRY * index << 4)
    .1
...
fn compute_combined_cost(
  ...
  stride_pdf.clone_from_slice(
    cdfs
      .split_at(NUM_SPEEDS_TO_TRY * nibble)
      .1
  ...
  if nibble_u8 != 0 {
    ...
    tmp.clone_from_slice(
      cdfs
        .split_at(NUM_SPEEDS_TO_TRY * (nibble - 1))
        .1
        .split_at(NUM_SPEEDS_TO_TRY)
        .0,
    );
  ...
  for i in 0..NUM_SPEEDS_TO_TRY {
    ...
    let combined_pdf =
      w * u32::from(stride_pdf[i]) + ((1 << BLEND_FIXED_POINT_PRECISION) - w) * u32::from(cm_pdf);
    let combined_max =
      w * u32::from(stride_max[i]) + ((1 << BLEND_FIXED_POINT_PRECISION) - w) * u32::from(cm_max);
  ...
  for nibble in 0..16 {
    for speed_index in 0..NUM_SPEEDS_TO_TRY {
      if nibble == 0 {
        ...
        assert!(
          cdfs[overall_index + speed_index] - cdfs[overall_index + speed_index - NUM_SPEEDS_TO_TRY]
            != 0
        );
      }
    }
  ...
  ContextMapEntropy<'a, Alloc>
{
  ...
  pub fn free(&mut self, alloc: &mut Alloc) {
    <Alloc as Allocator<u16>>::free_cell(
      alloc,
      core::mem::take(&mut self.cm_priors),
    );
    <Alloc as Allocator<u16>>::free_cell(
      alloc,
      core::mem::take(&mut self.stride_priors),
    );
  }
  fn update_cost_base(
    &mut self,

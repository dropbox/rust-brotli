...
use super::{
  kDistanceCacheIndex, kDistanceCacheOffset, kHashMul32, kHashMul64, kHashMul64Long, kInvalidMatch,
  AnyHasher, BrotliEncoderParams, BrotliHasherParams,
};
use alloc;
...
fn ZopfliCostModelSetFromLiteralCosts<AllocF: Allocator<floatX>>(
  ...
  while i < num_bytes {
    {
      literal_carry =
        literal_carry as floatX + literal_costs[(i.wrapping_add(1usize) as (usize))] as floatX;
...
fn InitDictionaryBackwardMatch(
  ...
  (*xself)
    .set_length_and_code((len << 5i32 | if len == len_code { 0usize } else { len_code }) as (u32));
}
  ...
  Buckets: PartialEq<Buckets>,
{
    ...
    && position >= Params::max_tree_comp_length() as usize)
  {
    ...
    for i in i_start..i_end {
      ...
      let max_backward = handle.window_mask_ - core::cmp::max(BROTLI_WINDOW_GAP - 1, position - i);
      let mut _best_len = 0;
  ...
  while i < len {
    {
        ...
        > (q[((offset.wrapping_add(1usize) & 7usize) as (usize))]).costdiff
      {
        ...
        q[((offset & 7usize) as (usize))] = q[((offset.wrapping_add(1usize) & 7usize) as (usize))];
          ...
          'continue30: loop {
            {
              ...
              let backward: usize =
                ((*posdata).distance_cache[(idx as (usize) & distance_cache_len_minus_1)]
                  + i32::from(kDistanceCacheOffset[(j as (usize))])) as (usize);
              let mut prev_ix: usize = cur_ix.wrapping_sub(backward);
              let len: usize;
              let continuation: u8 = ringbuffer[(cur_ix_masked.wrapping_add(best_len) as (usize))];
          ...
          while j < num_matches {
            {
              ...
              let is_dictionary_match: i32 = if !!(dist > max_distance.wrapping_add(gap)) {
                1i32
              } else {
                0i32
              };
              ...
              dist_cost = base_cost
                ...
                + ZopfliCostModelGetDistanceCost(model, (dist_symbol as (i32) & 0x3ff) as (usize));
              ...
              while len <= max_match_len {
                {
                  ...
                  let cost: floatX = dist_cost
                    + GetCopyExtra(copycode) as (floatX)
                    + ZopfliCostModelGetCommandCost(model, cmdcode);
                  if let Union1::cost(nodeCost) = (nodes[(pos.wrapping_add(len) as (usize))]).u {
                    if cost < nodeCost {
  ...
  while i.wrapping_add(handle.HashTypeLength()).wrapping_sub(1usize) < num_bytes {
    {
      ...
      {
        skip = brotli_max_size_t(BackwardMatchLength(&BackwardMatch(matches[(0usize)])), skip);
      }
      if skip > 1usize {
        handle.StoreRange(
          ringbuffer,
    ...
    'continue56: loop {
      {
        ...
        cost[(i as (usize))] = log2sum - FastLog2(u64::from(histogram[(i as (usize))])) as (floatX);
  ...
  while i < num_commands {
    {
      ...
      let distcode: usize = ((commands[(i as (usize))]).dist_prefix_ as (i32) & 0x3ff) as (usize);
      ...
      while j < inslength {
        {
          ...
          let _lhs = &mut histogram_literal
            [(ringbuffer[((pos.wrapping_add(j) & ringbuffer_mask) as (usize))] as (usize))];
          *_lhs = (*_lhs).wrapping_add(_rhs as (u32));
        }
        j = j.wrapping_add(1 as (usize));
      }
    ...
    while i < num_bytes {
      {
        literal_carry = literal_carry
          + cost_literal
            [(ringbuffer[((position.wrapping_add(i) & ringbuffer_mask) as (usize))] as (usize))]
            as floatX;
        literal_costs[(i.wrapping_add(1usize) as (usize))] =
          (literal_costs[(i as (usize))] as floatX + literal_carry) as floatX;
        literal_carry = literal_carry

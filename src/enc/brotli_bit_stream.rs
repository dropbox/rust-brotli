...
use super::constants::{
  kCodeLengthBits, kCodeLengthDepth, kCopyBase, kCopyExtra, kInsBase, kInsExtra, kNonZeroRepsBits,
  ...
  kZeroRepsDepth, BROTLI_CONTEXT_LUT, BROTLI_NUM_BLOCK_LEN_SYMBOLS, BROTLI_NUM_COMMAND_SYMBOLS,
  BROTLI_NUM_HISTOGRAM_DISTANCE_SYMBOLS, BROTLI_NUM_LITERAL_SYMBOLS,
};
use super::context_map_entropy::{speed_to_tuple, ContextMapEntropy, SpeedAndMax};
use super::entropy_encode::{
  BrotliConvertBitDepthsToSymbols, BrotliCreateHuffmanTree, BrotliSetDepth, BrotliWriteHuffmanTree,
  HuffmanComparator, HuffmanTree, InitHuffmanTree, NewHuffmanTree, SortHuffmanTreeItems,
};
...
impl<'a, Alloc: BrotliAlloc> interface::CommandProcessor<'a> for CommandQueue<'a, Alloc> {
  fn push(&mut self, val: interface::Command<InputReference<'a>>) {
    if self.full() {
      let mut tmp =
        <Alloc as Allocator<StaticCommand>>::alloc_cell(self.mc, self.queue.slice().len() * 2);
      tmp
        .slice_mut()
...
fn warn_on_missing_free() {
  let _err =
    ::std::io::stderr().write(b"Need to free entropy_tally_scratch before dropping CommandQueue\n");
}
#[cfg(not(feature = "std"))]
fn warn_on_missing_free() {
...
fn process_command_queue<'a, CmdProcessor: interface::CommandProcessor<'a>>(
  ...
  for cmd in commands.iter() {
    let (inserts, interim) = input_iter.split_at(core::cmp::min(cmd.insert_len_ as usize, mb_len));
...
fn BrotliStoreHuffmanTreeOfHuffmanTreeToBitMask(
  ...
  if num_codes > 1i32 {
    'break5: while codes_to_store > 0 {
      {
        if code_length_bitdepth[(kStorageOrder[codes_to_store.wrapping_sub(1) as usize] as (usize))]
          as (i32)
          != 0i32
        {
          {
  ...
  > MetaBlockSplit<Alloc>
{
  ...
  pub fn destroy(&mut self, alloc: &mut Alloc) {
    ...
    <Alloc as Allocator<u32>>::free_cell(alloc, core::mem::take(&mut self.distance_context_map));

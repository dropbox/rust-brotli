...
fn oneshot_compress(
  ...
  let stack_u16_buffer = unsafe { define_allocator_memory_pool!(96, u16, [0; 128 * 1024], calloc) };
  let stack_i32_buffer = unsafe { define_allocator_memory_pool!(96, i32, [0; 128 * 1024], calloc) };
  let stack_u32_buffer =
    unsafe { define_allocator_memory_pool!(96, u32, [0; 32 * 1024 * 1024], calloc) };
  let stack_u64_buffer = unsafe { define_allocator_memory_pool!(96, u64, [0; 32 * 1024], calloc) };
  ...
  let stack_zn_buffer = unsafe { define_allocator_memory_pool!(48, ZopfliNode, [0; 1024], calloc) };
  let stack_mc_buffer =
  ...
  let mhl = CallocatedFreelist2048::<HistogramLiteral>::new_allocator(stack_hl_buffer.data, bzero);
  let mhc = CallocatedFreelist2048::<HistogramCommand>::new_allocator(stack_hc_buffer.data, bzero);
  let mhd = CallocatedFreelist2048::<HistogramDistance>::new_allocator(stack_hd_buffer.data, bzero);
  ...
  let mut next_out_offset: usize = 0;
  {
    ...
    loop {
      ...
      let mut nop_callback = |_data: &mut interface::PredictionModeContextMap<

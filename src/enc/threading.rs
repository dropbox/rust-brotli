  ...
  <Alloc as Allocator<u8>>::AllocatedMemory: Send,
{
  ...
  pub fn unwrap_or(self, other: Alloc, other_extra: ExtraInput) -> (Alloc, ExtraInput) {
    match self.0 {
      ...
      InternalSendAlloc::SpawningOrJoining(_) | InternalSendAlloc::Join(_) => (other, other_extra),
    }
  }
  ...
  <Alloc as Allocator<u32>>::AllocatedMemory: Send,
{
  ...
  if num_threads > 1 && params.favor_cpu_efficiency {
    ...
    compression_last_thread_result = spawner_and_input.view(
      move |input_and_params: &(SliceW, BrotliEncoderParams)| -> CompressionThreadResult<Alloc> {
        compress_part(
          hasher,
          num_threads - 1,
          num_threads,

  ...
  while i < len {
    {
      if i >= window_half {
        ...
        let utf8_pos2: usize = UTF8Position(last_c, c, max_utf8);
        {
          ...
          let _lhs = &mut histogram[utf8_pos2]
            [data[((pos.wrapping_add(i).wrapping_sub(window_half) & mask) as (usize))] as (usize)];
      ...
      if i.wrapping_add(window_half) < len {
        ...
        let utf8_pos2: usize = UTF8Position(last_c, c, max_utf8);
        {
          let _rhs = 1;
          let _lhs = &mut histogram[utf8_pos2]
            [data[((pos.wrapping_add(i).wrapping_add(window_half) & mask) as (usize))] as (usize)];
          *_lhs = (*_lhs).wrapping_add(_rhs as (usize));
        }
        {
          let _rhs = 1;
      ...
      }
      {
        ...
        lit_cost =
          FastLog2f64(in_window_utf8[utf8_pos] as u64) as f64 - FastLog2f64(histo as u64) as f64;
        ...
        if i < 2000usize {
          lit_cost = lit_cost + (0.7 - (2000usize).wrapping_sub(i) as (f64) / 2000.0 * 0.35);
        }

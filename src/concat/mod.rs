      ...
      if self.window_size == 0 {
        ...
        *out_offset += 1;
        ...
        for byte_index in 0..var_len_bytes {
          ...
          realigned_header[byte_index] |= ((cur_byte
            & ((1 << (8 - self.last_byte_bit_offset)) - 1))
        ...
        let whole_byte_destination =
          ((usize::from(self.last_byte_bit_offset) + varlen_offset - window_offset) + 7) / 8;
  ...
  pub fn stream(
    ...
    if let Some(mut new_stream_pending) = self.new_stream_pending.clone() {
      ...
      if let BroCatliResult::Success = flush_result {
        ...
        let shift_result =
          self.shift_and_check_new_stream_header(new_stream_pending, out_bytes, out_offset);
    ...
    let mut to_copy = core::cmp::min(out_bytes.len() - *out_offset, in_bytes.len() - *in_offset);
    assert!(to_copy != 0);
    if to_copy == 1 {
      out_bytes[*out_offset] = self.last_bytes[0];
      self.last_bytes[0] = self.last_bytes[1];

...
use brotli::{DecompressorWriterCustomIo, HuffmanCode, CustomWrite};
...
pub fn compress_validate<InputType:Read, OutputType: Write>(r: &mut InputType,
    ...
    let mut validate_writer = DecompressAndValidate(DecompressorWriterCustomIo::new_with_custom_dictionary(
    ...
    let ret = super::compress(r, &mut overarching_writer, buffer_size, params, &dict[..], num_threads);
    match ret {
        //FIXME: check sig here
        Ok(ret) => Ok(()),
        Err(e) => Err(e),
    }
}
